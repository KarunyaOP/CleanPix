...
export const Hero: React.FC = () => {
  ...
  return (
    <section className="relative pt-6 sm:pt-12 pb-24 sm:pb-32 overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-10 items-center">
          ...
          <div className="lg:col-span-6 xl:col-span-5 flex flex-col items-start text-left z-10">
            {file && previewUrl ? (
              ...
              <>
                {/* Eyebrow Pill Badge */}
                <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-pill bg-[#131A3A]/80 border border-primary/35 backdrop-blur-xl shadow-[0_0_20px_rgba(79,124,255,0.25)] mb-6 animate-in fade-in duration-300">
                ...
                {/* Main Headline */}
                <h1 className="font-heading font-extrabold text-4xl sm:text-5xl lg:text-[58px] leading-[1.1] tracking-tight text-[#F8FAFC] mb-6">
                ...
                {/* Subcopy */}
                <p className="text-base sm:text-lg text-text-secondary leading-relaxed max-w-lg mb-8 font-normal">
                ...
                <div
                  ...
                  className={`w-full rounded-[22px] border-2 transition-all duration-200 cursor-pointer p-5 sm:p-6 mb-3 relative overflow-hidden group select-none ${
                    isDragging
                    ...
                    <div className="flex flex-col items-start gap-4">
                      <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                        ...
                        <button
                          ...
                          className="inline-flex items-center justify-center gap-2 px-5 py-3.5 rounded-btn font-medium text-xs sm:text-sm text-[#F8FAFC] bg-[#0A0B1E]/70 border border-white/15 backdrop-blur-md hover:border-accent/40 hover:bg-[#1B2350]/90 hover:shadow-[0_0_20px_rgba(34,211,238,0.2)] transition-all duration-200 cursor-pointer"
                        >
                      ...
                      <div className="flex items-center gap-2 text-xs text-text-muted font-medium tracking-wide">
                        <span className="w-1.5 h-1.5 rounded-full bg-accent/70" />
                        <span>or drag and drop your image anywhere in this box (Ctrl + V supported)</span>
                      </div>
                    </div>
                  )}
                </div>
                ...
                {/* Format & Size Caption */}
                <p className="text-xs text-text-muted font-medium mb-10 pl-1 tracking-wide">
                  JPG, PNG, WEBP <span className="mx-1">•</span> Max 10MB
                </p>

                {/* 4-Item Feature Icons / Trust Row */}
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6 w-full pt-6 border-t border-white/[0.1]">
                  {trustFeatures.map((feature) => {
                    const Icon = feature.icon;
                    return (
                      <div
                        key={feature.id}
                        className="flex flex-col items-start gap-2.5 group cursor-default"
                      >
                        <div className="w-9 h-9 rounded-chip bg-primary/15 border border-primary/30 flex items-center justify-center text-primary group-hover:text-accent group-hover:border-accent/50 group-hover:shadow-[0_0_16px_rgba(79,124,255,0.4)] transition-all duration-200">
                          <Icon size={18} />
                        </div>
                        <span className="text-xs font-semibold text-text-secondary group-hover:text-[#F8FAFC] transition-colors leading-tight">
                          {feature.title}
                        </span>
                      </div>
                    );
          ...
          <div className="lg:col-span-6 xl:col-span-7 flex items-center justify-center lg:justify-end z-10 mt-6 lg:mt-0">
            {file && previewUrl ? (
              ...
              <div className="w-full flex flex-col gap-4">
                <ProcessedPreviewCard
                  ...
                  onCopyClipboard={() => copyToClipboard(processedUrl || undefined)}
                  detectedCategory={detectedObject}
                  showCopySection={true}
                />
              </div>
            ) : (
              /* LANDING PAGE: Demo Preview + Compact Smart Recommendation Panel ONLY (NO Copy Image Section) */
              <div className="w-full max-w-[620px] flex flex-col gap-4">
                ...
                <LandingDetectedPanel
                  ...
                  paddingPercent={landingPadding}
                  onPaddingChange={setLandingPadding}
                />
              </div>
            )}
          </div>
